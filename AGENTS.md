...
| `src/v2/` | v2 evidence-plane primitives: canonical JSON, bounded JSON admission, and Ed25519 key/signature handling (`crypto.subtle` only). Read-side building blocks; not yet wired into publishing. |
| `src/v2view/` | The portal's build-time view of the v2 register: runs the `src/v2/` client from a pinned root, verifies records against the TUF-authenticated policy, and writes `model-v2.generated.json`. Imported by the build only, never by `worker.ts`. |
