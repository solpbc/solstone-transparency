...
**Status: the read-side legacy verifier/model exists; a read-only HTML presentation layer exists under `src/portal/`; it is deployed and live at `trust.solstone.app` (`worker.ts` wires `src/portal` over a model built by `make build-model` immediately before deploy).** `src/legacy/` fetches the existing historical (v1) release-transparency register, verifies each record (minisign signature, hash-chain linkage, schema shape), and builds a typed, already-verified model of the register: see that directory's own module comments for the exact contract. `src/portal/` renders that model as server-side HTML. The v1 implementation is read-only. `src/v2/` and `bin/` implement TUF repository construction, DSSE records and authorization, conditional publication, and release verification. Tests use synthetic keys; production signing and publishing credentials must remain outside this repository. `src/v2view/` is the portal's build-time view of the v2 register: `make build-model` verifies the v2 repository from a pinned root file (`protocol/tuf-root.json`, absent until the root exists) and writes `model-v2.generated.json` beside the v1 model; with no pinned root the portal says nothing about v2; a repository that does not verify is shown as unverified, and never as a fabricated register. See [`README.md`](README.md) for the current install/run/test instructions.
...
| `src/v2/` | TUF and DSSE verification, repository signing, encrypted ceremony keys, release records, conditional publication, and timestamp refresh. Publication is separate from artifact delivery. |
| `src/v2view/` | The portal's build-time view of the v2 register: runs the `src/v2/` client from a pinned root, verifies records against the TUF-authenticated policy, and writes `model-v2.generated.json`. Imported by the build only; `worker.ts` embeds its output and imports only its types. |
| `bin/` | `solstone-transparency.ts` and operator entry points; consult each command’s `--help` |
| `protocol/` | Public evidence-record and predicate documents, plus conformance fixtures. Read its `README.md` and the licensing note in [`CONTRIBUTING.md`](CONTRIBUTING.md) before adding anything here. |
...
- **No hidden dependencies on proprietary infrastructure.** The verifier and the portal must stay buildable and runnable by a third party with no relationship to sol pbc.
- **Do not build ahead of what's approved.** This repository implements only what has an approved contract. New evidence types, verification paths and portal routes each land as their own reviewed change against an actual approved contract, not speculatively.
...
- **Treat dependencies as attack surface.** Prefer zero runtime dependencies where the task allows it, as this repository currently does. Before adding one that's genuinely needed, read its actual license text yourself rather than trusting a package-manager metadata field, and keep the tree small.
...
- **No CI/CD on GitHub, no exception.** This is absolute, not a preference: no `.github/workflows/` for tests, linting, or releases, ever. A pull request adding a workflow file gets declined regardless of what it does.
...
The prohibition applies to every contribution, including temporary debugging inputs:
...
- Do not add sample "evidence," parsing, verification, or routing logic speculatively. A change that needs one of those needs its own approved scope first.
...
- Every `.ts` source file, but not docs, config, generated files, or vendored files, carries an SPDX header immediately after any shebang:
  ...
  ```
- "Done" means the change's own acceptance criteria are demonstrated and `make ci` is green, not merely that the build passes.
