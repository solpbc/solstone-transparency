...
| `src/v2/` | v2 evidence-plane primitives: canonical JSON, bounded JSON admission, and Ed25519 key/signature handling (`crypto.subtle` only). Includes the `publish-v2` command, which signs and writes a repository to a local directory; nothing here uploads to the evidence host. |
