...
import { runAuditV2 } from "../bin/audit-v2";
import { publicationMain } from "../bin/publish-transaction";
import { runVerifyRelease } from "../bin/verify-release";
import { VERSION } from "./index";
import { buildPortalModel } from "./legacy/adapter";
import { buildSitemap } from "./portal/sitemap";
import type { PassphraseSource } from "./v2/ceremony-key";
import {
	loadPassphraseProvider,
	passphraseProviderOption,
} from "./v2/passphrase-provider";
import {
...
import { loadEncryptedSigningKeys } from "./v2/tuf/encrypted-keyset";
import {
	RELEASE_SIGNING_KEY_FIELDS,
	incrementalSigningKeys,
	metadataRenewalKeyFields,
...
Options:
  ...
  publish-v2          Build a local v2 fixture repository containing
                      ...
                      Input shapes:
                        ...
                        --keys <path>: JSON object with 9 signing keys:
                          - root: array of 1 key entry
                        ...
                        --policy-sha256 <hex>: 64-character lowercase hex
                          digest supplied by the fixture author. Use release prepare
                          to bind the authenticated policy for an incremental release.
  ...
  release prepare     Prepare one incremental release. --keys <path>: exactly
                      {"targets-software":"FILE","snapshot":"FILE",
                      "timestamp":"FILE","producer.release":"FILE"}.
                      --release-record
  ...
  metadata renew      Renew snapshot, targets, or one delegated targets role plus
                      ...
                      Snapshot keys are exactly {"snapshot":"FILE",
                      "timestamp":"FILE"}. Any targets-role keys are exactly
                      {"<role>":"FILE","snapshot":"FILE","timestamp":"FILE"}.

For release prepare, timestamp refresh, and metadata renew, FILE names an encrypted
Ed25519 PKCS#8 file, resolved from the current working directory. Plaintext keys and
key-entry JSON are refused. Public keys and key IDs are derived from decrypted keys.
Passphrases default to terminal input without echo. --passphrase-provider MODULE
selects trusted local code whose default function accepts a key path and returns
Promise<Buffer>; the reader clears that buffer after use. Never put a passphrase in
arguments or environment variables.

Preparation commands write locally. A successful preparation is not a claim of
public publication. Use the encrypted-key driver in bin/tuf-ceremony.ts for genesis
and bin/root-renew.ts for detached root renewal.
`;
...
async function readJson(
	...
	try {
		...
		inputError(`could not read or parse ${label}`);
		return undefined;
	}
}
...
function inputError(message: string): void {
	console.error(
		JSON.stringify({
			...
			reason: "malformed",
			message,
			recovery: "Check the input format and use --help for options.",
		}),
	);
}

async function preparationPassphraseSource(
	argv: readonly string[],
): Promise<PassphraseSource | undefined> {
	try {
		const selected = passphraseProviderOption(argv);
		return await loadPassphraseProvider(selected.modulePath);
	} catch {
		inputError(
			"could not select or load --passphrase-provider; supply one local module with a default function export",
		);
		return undefined;
	}
}
...
function nowFromFlag(value: string | undefined): Date | undefined {
	...
	if (!Number.isFinite(now.getTime())) {
		inputError("--now must be a valid ISO-8601 instant");
		return undefined;
	}
	return now;
}
...
function reportRejection(
	...
	console.error(
		JSON.stringify({
			...
			recovery: recoveryGuidance(result.reason),
		}),
	);
	return 1;
}
...
function requiredFlag(
	...
	if (value === undefined || value === "") {
		inputError(`${command} requires ${name} <value>`);
		return undefined;
	}
	return value;
}
...
export async function run(argv: string[]): Promise<number> {
	if (argv[0] === "verify-release") return runVerifyRelease(argv.slice(1));
	if (argv[0] === "audit-v2") return runAuditV2(argv.slice(1));
	if (argv[0] === "publish-transaction") return publicationMain(argv.slice(1));
	if (argv[0] === "--version") {
		console.log(VERSION);
		return 0;
	}
	...
	if (argv[0] === "policy" && argv[1] === "build") {
		...
		if (!Number.isSafeInteger(version) || version <= 0) {
			inputError("policy build --version must be a positive safe integer");
			return 1;
		}
	...
	if (argv[0] === "release" && argv[1] === "prepare") {
		...
		const source = await preparationPassphraseSource(argv);
		if (source === undefined) return 1;
		const keys = await loadEncryptedSigningKeys(
			rawKeys,
			RELEASE_SIGNING_KEY_FIELDS,
			source,
		);
		...
		const prepared = await prepareIncrementalRelease({
			...
			keys: incrementalSigningKeys(keys.value),
			now,
		});
		...
		const written = await writePreparationOutput({
			...
			priorState: prior.value,
			verification: { repositoryDirectory, rootPath, now },
			expectedPriorTimestampSha256: expectedTimestampSha256,
	...
	if (argv[0] === "timestamp" && argv[1] === "refresh") {
		...
		const source = await preparationPassphraseSource(argv);
		if (source === undefined) return 1;
		const keys = await loadEncryptedSigningKeys(rawKeys, ["timestamp"], source);
		if (!keys.ok) return reportRejection("timestamp refresh", keys);
		const timestampKey = keys.value.timestamp;
		if (timestampKey === undefined)
			throw new Error("exact encrypted key map omitted timestamp");
		const prior = await authenticateLocalRepository({
			repositoryDirectory,
			rootPath,
			expectedTimestampSha256,
