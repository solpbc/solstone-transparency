...
export async function prepareIncrementalRelease(
	...
	const forbiddenKeyids = new Set([
		...root.value.roles.root.keyids,
		...root.value.roles.targets.keyids,
	]);
	for (const field of INCREMENTAL_SIGNING_KEY_FIELDS) {
		const key = input.keys[field];
		const keyid = await computeKeyId(key.keyObject);
		if (!keyid.ok) return keyid;
		if (forbiddenKeyids.has(keyid.value)) {
			return rejection("role-not-authorized", {
				path: ["keys", field],
				expected: "a release key outside root and top-level targets authority",
				observed: keyid.value,
			});
		}
		if (keyid.value !== key.keyId) {
			return rejection("keyid-mismatch", {
				path: ["keys", field],
				expected: keyid.value,
				observed: key.keyId,
			});
		}
	}
